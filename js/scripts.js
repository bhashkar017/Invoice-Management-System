...
$(document).ready(function() {
	...
	function actionCreateInvoice(){
		...
		if (errorCounter > 0) {
			...
			// Simple AJAX call without any copy functionality
			$.ajax({
				url: 'response.php',
				type: 'POST',
				...
				success: function(data){
					if (data && data.status) {
						...
						$("#response .message").html("<strong>Error</strong>: Invalid response from server");
						$("#response").removeClass("alert-success").addClass("alert-warning").fadeIn();
					}
					$btn.button("reset");
				},
				error: function(xhr, status, error){
					var errorMessage = "An error occurred while creating the invoice. ";
					if (xhr.responseText) {
						try {
