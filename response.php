...
if ($action == 'create_invoice'){
    ...
    if (empty($customer_name) || empty($invoice_number) || empty($invoice_date) || empty($invoice_due_date)) {
        echo json_encode(array(
            'status' => 'Error',
            'message' => 'Required fields are missing. Please check customer name, invoice number, and dates.'
        ));
        exit;
    }

    // Check if invoice number already exists
    $check_query = "SELECT invoice FROM invoices WHERE invoice = ?";
    ...
    if($stmt->execute()){
        ...
        if($customer_stmt === false) {
            trigger_error('Wrong SQL: ' . $customer_query . ' Error: ' . $mysqli->error, E_USER_ERROR);
        }
        ...
        if($customer_stmt->execute()){
            ...
            echo json_encode(array(
                'status' => 'Error',
                'message' => 'There was an error saving customer information.'
            ));
        }
    } else {
        // Error response
